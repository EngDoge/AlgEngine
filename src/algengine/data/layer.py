...
def extract_target_layers(mask: np.ndarray,
    ...
    return {layer: extract_layer(mask=mask, layer_id=idx) for layer, idx in layers.items()}
...
class MaskLayer:
    ...
    def from_SingleImage(cls,
        ...
        return cls(data=data, name=osp.splitext(single_image.name)[0] if name is None else name)
